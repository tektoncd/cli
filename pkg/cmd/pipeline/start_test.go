...
func TestPipelineStart_ComprehensiveResolverValidation(t *testing.T) {
	tests := []struct {
		...
		// Invalid resolver type validation
		{
			...
			expectedErr: "invalid resolvertype 'invalid'. Valid values are: hub, git, http, cluster, bundles, remote",
		...
		{
			name:        "bundles resolver without pipeline name",
			args:        []string{"start", "--resolvertype=bundles"},
			expectedErr: "pipeline name is required when using --resolvertype flag",
			description: "bundles resolver should require pipeline name",
		},
		{
		...
		// --resolvertype with pipeline name (without --last)
		{
			...
			expectedErr: "PipelineRun started:", // Should succeed and create PipelineRun
			description: "Hub resolver with pipeline name should work",
		},
		{
			name:        "git resolver with pipeline name",
			args:        []string{"start", "test-pipeline", "--resolvertype=git"},
			expectedErr: "PipelineRun started:", // Should succeed and create PipelineRun
			description: "Git resolver with pipeline name should work",
		},
		{
		...
		},
		{
			...
			expectedErr: "no pipelineruns related to pipeline test-pipeline found in namespace",
			description: "Resolver + last with pipeline name should work",
		},
		{
