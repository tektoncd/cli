...
func TestListTriggerBinding_structured_output(t *testing.T) {
	now := time.Now()
	ns := []*corev1.Namespace{
		{
			ObjectMeta: metav1.ObjectMeta{
	...
	var fromJSON, fromYAML []v1beta1.TriggerBinding
	...
	if len(fromJSON) != 2 || len(fromYAML) != 2 {
		t.Errorf("expected 2 triggerbindings, got json=%d yaml=%d", len(fromJSON), len(fromYAML))
	}
	if strings.Contains(jsonOut, "TriggerBindingList") {
		t.Errorf("json output should be an array of TriggerBinding objects, not a TriggerBindingList wrapper")
	}
}

func TestListTriggerBinding_empty_output_json(t *testing.T) {
	now := time.Now()
	ns := []*corev1.Namespace{
		{
			ObjectMeta: metav1.ObjectMeta{
	...
	p := command(t, []*v1beta1.TriggerBinding{}, now, ns)
	output, err := test.ExecuteCommand(Command(p), "list", "-n", "foo", "-o", "json")
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}

	var got []v1beta1.TriggerBinding
...
func TestListTriggerBinding_invalid_output(t *testing.T) {
	now := time.Now()
	ns := []*corev1.Namespace{
		{
			ObjectMeta: metav1.ObjectMeta{
...
func TestListTriggerBinding_help_shows_output_examples(t *testing.T) {
