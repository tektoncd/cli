...
func Command() *cobra.Command {
	eg := `Create or update resources defined in foo.yaml in namespace 'bar':
...
`
	...
	c := &cobra.Command{
		Use:          "apply",
		Short:        "Uses kubectl apply to create and update Kubernetes resources",
		Example:      eg,
		SilenceUsage: true,
...
func invokeCommand(command string) runCmd {
	return func(_ *cobra.Command, _ []string) {
		...
		if !isKubectlAvailable() {
			log.Print("error: kubectl is not available. kubectl must be installed to use tkn apply.")
			return
		}
