...
func createCommand(p cli.Params) *cobra.Command {
	...
	tkn task create --from-clustertask foo
or
	tkn task create foobar --from-clustertask=foo -n ns`
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if opts.FromClusterTask == "" {
				return errors.New("--from-clustertask flag not passed")
			}

			if len(args) != 0 {
				return createTaskFromClusterTask(s, p, args[0], opts.FromClusterTask)
			}
			return createTaskFromClusterTask(s, p, opts.FromClusterTask, opts.FromClusterTask)

		},
	}
	c.Flags().StringVarP(&opts.FromClusterTask, "from-clustertask", "", "", "Create a ClusterTask from Task in a particular namespace")
	return c
}
