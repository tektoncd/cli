...
**If the user specified a version** (e.g. "create release note v0.45.0"), use that directly as the current tag — skip auto-detection. Validate that the tag exists locally:
...
- Release infrastructure (release scripts, goreleaser config)
- Test infrastructure (test helpers, e2e framework, test configuration)
- Build system changes (Dockerfile, build scripts)
...
Only classify as Features when the change is **visible to end users**: new CLI commands, new flags, new output formats, new plugin capabilities, new resource support.
