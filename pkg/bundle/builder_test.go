...
func TestBuildTektonBundle(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Failed to read image layer: %s", err.Error())
	}
	defer rc.Close()
	...
	if _, err := treader.Read(contents); err != nil && err != io.EOF {
		...
		t.Errorf("failed to read tar bundle: %s", err.Error())
	}

	obj, _, err := scheme.Codecs.UniversalDeserializer().Decode(contents, nil, nil)
	if err != nil {
		t.Errorf("failed to decode layer contents to a Tekton object: %s", err.Error())
	}
