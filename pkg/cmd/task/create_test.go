...
func TestCreate_ClusterTaskNotExist(t *testing.T) {
	...
	out, err := test.ExecuteCommand(clustertask, "create", "task", "--from-clustertask", "ct-2")
	if err == nil {
...
func TestCreate(t *testing.T) {
	...
	tdc := testDynamic.Options{}
	dynamic, err := tdc.Client(
		cb.UnstructuredCT(ctasks[0], version),
	)
	if err != nil {
		t.Errorf("unable to create dynamic client: %v", err)
	}

	cs, _ := test.SeedTestData(t, pipelinetest.Data{ClusterTasks: ctasks})
	p := &test.Params{Tekton: cs.Pipeline, Kube: cs.Kube, Dynamic: dynamic}
	p.SetNamespace("default")
	cs.Pipeline.Resources = cb.APIResourceList(version, []string{"task", "clustertask"})
	...
	out, err := test.ExecuteCommand(clustertask, "create", "t-1", "--from-clustertask", "ct-1")
	if err != nil {
...
func TestCreate_InNamespace(t *testing.T) {
	...
	cs, _ := test.SeedTestData(t, pipelinetest.Data{ClusterTasks: ctasks, Namespaces: namespaces})
	p := &test.Params{Tekton: cs.Pipeline, Kube: cs.Kube, Dynamic: dynamic}
	cs.Pipeline.Resources = cb.APIResourceList(version, []string{"task", "clustertask"})

	clustertask := Command(p)

	out, err := test.ExecuteCommand(clustertask, "create", "--from-clustertask", "ct-1", "-n", "ns")
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
...
func TestCreate_V1beta1(t *testing.T) {
...
func TestCreate_WithoutFlag(t *testing.T) {
	...
	expected := "Error: --from-clustertask flag not passed\n"
	test.AssertOutput(t, expected, out)
}
