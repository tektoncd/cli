...
type managedObj struct {
	metav1.ObjectMeta
}

func TestStripManagedFields(t *testing.T) {
	original := managedObj{ObjectMeta: metav1.ObjectMeta{
		Name:          "foo",
		ManagedFields: []metav1.ManagedFieldsEntry{{Manager: "kubectl"}},
	}}

	got := StripManagedFields(original)

	assert.Equal(t, 0, len(got.ManagedFields))
	assert.Equal(t, 1, len(original.ManagedFields))
	assert.Equal(t, "kubectl", original.ManagedFields[0].Manager)
}

func TestStripManagedFields_nonObject(t *testing.T) {
	assert.Equal(t, "foo", StripManagedFields("foo"))
}

func TestStripManagedFieldsList(t *testing.T) {
	items := []managedObj{
		...
		{ObjectMeta: metav1.ObjectMeta{
			Name:          "bar",
			ManagedFields: []metav1.ManagedFieldsEntry{{Manager: "tkn"}},
		}},
	}

	stripped := StripManagedFieldsList(items, false)
	assert.Equal(t, 2, len(stripped))
	assert.Equal(t, 0, len(stripped[0].ManagedFields))
	assert.Equal(t, 0, len(stripped[1].ManagedFields))
	assert.Equal(t, 1, len(items[0].ManagedFields))
	assert.Equal(t, 1, len(items[1].ManagedFields))

	kept := StripManagedFieldsList(items, true)
	assert.Equal(t, 1, len(kept[0].ManagedFields))
	assert.Equal(t, "kubectl", kept[0].ManagedFields[0].Manager)
}

func TestStripManagedFieldsList_empty(t *testing.T) {
	got := StripManagedFieldsList([]managedObj{}, false)
