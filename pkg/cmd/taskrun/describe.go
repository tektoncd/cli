...
`
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
				...
				if !opts.Last {
					...
					if len(trs) == 0 {
						if formatted.IsStructured(output) {
							return fmt.Errorf("no TaskRuns present in namespace %s", opts.Params.Namespace())
						}
			...
			if formatted.IsStructured(output) {
				tr, err := taskrunpkg.GetTaskRun(taskrunGroupResource, cs, opts.TaskrunName, p.Namespace())
				if err != nil {
					return err
				}
				items := []v1.TaskRun{*tr}
				...
				if !f.JSONYamlPrintFlags.ShowManagedFields {
					obj = formatted.StripManagedFields(obj)
				}
				return formatted.PrintStructuredOutput(cmd.OutOrStdout(), output, obj)
			}
